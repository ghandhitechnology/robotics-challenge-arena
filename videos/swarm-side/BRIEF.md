...
Replay the saved native MuJoCo trajectory from a fixed low oblique camera. A narrow evidence rail reports the verified transport result without applying the official competition score.
...
- Native MuJoCo rendering through a fixed render-only camera whose complete pose is recorded in the manifest.
- Fixed low oblique framing that keeps the field and payload grips useful to inspect.
